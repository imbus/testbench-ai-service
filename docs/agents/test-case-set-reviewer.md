...
|-------------|-------------|
| `agent.test_case` | Formatted string representation of the test case set (name, steps, parameters). |
| `agent.parameter_combinations` | Formatted parameter combination table. May be empty. |
| `agent.test_case_set_description` | The existing test case set description, if one is present (plain text, stripped of HTML). May be empty. |
...
|----------|------|-------------|
| `vars.glossary` | `text` | Optional glossary of project-specific terms. When provided, test steps are checked against it. |
