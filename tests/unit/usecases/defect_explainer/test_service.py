...
class TestDefectExplainerPrecheck(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_unlocked_tcs_passes_and_is_included(self):
        ...
        with (
            patch(
                "testbench_ai_service.agents.defect_explainer.agent.get_test_case_set_catalog",
                return_value={"TCS1": tcs},
            ),
            patch(
                "testbench_ai_service.agents.defect_explainer.agent.check_test_case_set_is_locked",
                return_value=False,
            ),
        ):
            result = await self.service.precheck(context, conn)
    ...
    async def test_http_error_is_handled(self):
        ...
        with (
            ...
            patch(
                "testbench_ai_service.agents.defect_explainer.agent.handle_requests_http_error"
            ) as mock_handler,
        ):
