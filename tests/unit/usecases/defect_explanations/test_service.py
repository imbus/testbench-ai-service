...
class TestDefectExplainerPrecheck(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_returns_failed_when_cycle_key_is_none(self):
        context = _make_context(cycle_key=None)
        conn = MagicMock()
    ...
    async def test_unlocked_tcs_passes_and_is_included(self):
        ...
        with (
            ...
            patch(
                "testbench_ai_service.usecases.defect_explanations.service.check_test_case_set_is_locked",
                return_value=False,
            ),
        ):
            result = await self.service.precheck(context, conn)
    ...
    async def test_http_error_is_handled(self):
        ...
        with (
            ...
            patch(
                "testbench_ai_service.usecases.defect_explanations.service.handle_requests_http_error"
            ) as mock_handler,
        ):
...
class TestBuildPlaceholderData(unittest.TestCase):
    ...
    def test_contains_failed_test_case_and_error_message(self):
        ...
        with patch(
            "testbench_ai_service.usecases.defect_explanations.service.get_test_case_set_as_string",
...
class TestGetAiResponse(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_wraps_llm_output_in_use_case_result(self):
        ...
        with patch(
            "testbench_ai_service.usecases.defect_explanations.service.build_prompt"
        ) as mock_build:
