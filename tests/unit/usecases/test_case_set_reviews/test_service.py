...
class TestTestCaseSetReviewerPrecheck(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_returns_passed_true_for_unlocked_tcs(self):
        ...
        with (
            patch(
                "testbench_ai_service.usecases.test_case_set_reviews.service.get_test_case_set_catalog",
                return_value={"TCS1": tcs},
            ),
            patch(
                "testbench_ai_service.usecases.test_case_set_reviews.service.check_test_case_set_is_locked",
                return_value=False,
            ),
        ):
    ...
    async def test_http_error_is_handled(self):
        ...
        with (
            ...
            patch(
                "testbench_ai_service.usecases.test_case_set_reviews.service.handle_requests_http_error"
            ) as mock_handler,
        ):
...
class TestBuildPlaceholderData(unittest.TestCase):
    ...
    def test_contains_required_keys(self):
        ...
        with (
            patch(
                "testbench_ai_service.usecases.test_case_set_reviews.service.get_test_case_set_as_string",
                return_value="test case string",
            ),
            patch(
                "testbench_ai_service.usecases.test_case_set_reviews.service.get_parameter_combinations_as_string",
                return_value="| col | val |",
            ),
            patch(
                "testbench_ai_service.usecases.test_case_set_reviews.service.get_test_case_glossary",
    ...
    def test_description_included_when_present(self):
        ...
        with (
            ...
            patch(
                "testbench_ai_service.usecases.test_case_set_reviews.service.extract_text_from_html_body",
...
class TestGetAiResponse(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_returns_use_case_result(self):
        ...
        with patch(
            "testbench_ai_service.usecases.test_case_set_reviews.service.build_prompt"
        ) as mock_build:
